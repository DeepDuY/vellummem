...
```
返回按真实余弦相似度（0~1）排序的结果，每项包含 `create_timestamp`。
...
vellum/
├── __init__.py               # 版本号
├── server.py                 # MCP 入口 + 9 工具 + @_tool
├── db.py                     # SQLite 连接 + 初始化
├── errors.py                 # 异常层次
├── groups.py                 # CPM k=3 分组
├── stores/
│   ├── __init__.py
│   └── human_timeline.py     # 记忆 CRUD + 分片
└── vector/
    ├── __init__.py
    └── adapter.py            # 向量引擎 + 预合并向量
schemas/
└── schema.sql
tests/
├── __init__.py
